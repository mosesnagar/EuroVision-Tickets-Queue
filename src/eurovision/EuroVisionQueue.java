...
public class EuroVisionQueue {
    ...
    public Customer servePriorityCustomer() {
        ...
        if (customer.prev == null) {
            q.first = customer.next;
            if (q.first != null) {
                this.q.first.prev = null;
            }
        } else if (customer.next == null) {
            this.q.last = customer.prev;
            this.q.last.next = null;
        } else {
            CustomerElement new_customer = customer.next;
            customer.prev.next = new_customer;
            new_customer.prev = customer.prev;
        }
